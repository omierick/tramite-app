...
const AreaManagement = () => {
  ...
  const handleCrearArea = async () => {
    ...
    const { error } = await supabase.from("areas").insert([{ nombre, descripcion }]);
    if (error) return alert("Error al guardar el área.");
    fetchAreas();
    setNombre("");
    setDescripcion("");
  };
  ...
  const guardarEdicion = async () => {
    if (!editData.nombre) return alert("El nombre es obligatorio.");
  ...
  const eliminarArea = async (id_area) => {
    ...
    if (error) alert("No se pudo eliminar el área.");
    else fetchAreas();
  };
  ...
  const areasFiltradas = areas
    ...
    .sort((a, b) => {
      ...
      if (typeof aVal === "string") return orden.asc ? aVal.localeCompare(bVal) : bVal.localeCompare(aVal);
      return 0;
    });
  ...
  const ordenarPor = (columna) => {
    setOrden((prev) => ({ columna, asc: prev.columna === columna ? !prev.asc : true }));
  };
  ...
  return (
    <div className="area-container">
      ...
      <div className="area-pagination">
        <button onClick={() => cambiarPagina(paginaActual - 1)} disabled={paginaActual === 1} className="area-page-btn">
          <FaChevronLeft />
        </button>
        ...
        <button onClick={() => cambiarPagina(paginaActual + 1)} disabled={paginaActual === totalPaginas} className="area-page-btn">
          <FaChevronRight />
        </button>
      </div>
      ...
      {editAreaId && (
        <div className="area-modal-overlay">
          <div className="area-modal-content">
            ...
            <input className="area-input" name="id_area" value={editData.id_area} disabled />
